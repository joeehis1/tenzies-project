...
function App() {
  ...
  const [diceCollection, setDiceCollection] = useState(generateDiceCollection())
  const [gamesPlayed, setGamesPlayed] = useState(function(){
  ...
  function rollDice(){
    ...
    setDiceCollection(collection => {
        const newCollection = [...collection].map((die) => {
            const newDie = {...die}
            newDie.number = newDie.isHeld ? newDie.number : Math.floor(Math.random()*6) +1
            return newDie
        })
  ...
  useEffect(()=>{
    ...
    const allEqual = diceCollection.every((die) => {
      return die.isHeld === true && die.number === diceCollection[0].number
    })
  ...
  function updateGamesPlayed(){
    ...
    setGamesPlayed(games => {
      return [...games, newGame].sort((a,b)=>{
        return a.gameDuration < b.gameDuration ? -1 :
              a.gameDuration > b.gameDuration ? 1 : 0
      })
    })
  }
  ...
  return (
    <>
      <div className="app-wrapper">
        ...
        {gamesPlayed.length > 0 && <ScoreTable isVisible={isVisible} gamesPlayed={gamesPlayed} />}
      </div>
